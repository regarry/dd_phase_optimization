...
def get_airy_psf_slice(r_grid, r_vec, z, wavelength, numerical_aperture, refractive_index, pixel_size):
    """
    Generates a single PSF slice using the paraxial (Fresnel) approximation.
    """
    # Constants
    ...
    # --- PHYSICS FIXES ---
    # 1. Transverse component (Bessel argument): k * NA * r
    # (Removed division by 'n' which was incorrect)
    arg_bessel = (k * NA) * r_vec[:, None] * rho[None, :]
    
    # 2. Axial component (Defocus phase): k * z * (NA/n)^2
    # Note: 0.5 factor comes from paraxial approximation sin(theta) ~ theta
...
def generate_psf(config, results_dir):
    ...
    z_step = config.get('px') 
    z_depth = np.arange(0, z_stop + z_step/1000.0, z_step) 
    ...
    for i, z in enumerate(z_depth):
        raw_slice = get_airy_psf_slice(r_grid_meters, r_vec, z, wavelength, NA, n, pixel_size)
        raw_slice *= mask 
    ...
    # --- NORMALIZATION FIX ---
    # Normalize the ENTIRE stack by the global maximum (usually at z=0)
    # This preserves relative intensity drop-off as you defocus.
    ...
    for i in range(len(psf_stack)):
        psf_8bit = np.clip(psf_stack[i] * 255, 0, 255).astype(np.uint8)
...
def apply_blur_kernel(image, psf_slice):
    """
    Convolves image with PSF using energy conservation.
    """
    if np.sum(psf_slice) == 0:
        return image
        
    # Normalize kernel sum to 1.0 to preserve total energy of the bead
    psf_norm = psf_slice / np.sum(psf_slice)
    return fftconvolve(image, psf_norm, mode='same')
